...
Every participant slot is a sub-slot of a DCAT-AP+ slot, following the [extension rules](https://nfdi-de.github.io/dcat-ap-plus/how-to-extend/). Input substances sub-slot → `had_input_entity`; output substances sub-slot → `had_output_entity`; agents (catalyst, solvent, reactor) sub-slots → `carried_out_by`.
