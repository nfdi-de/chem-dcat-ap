...
# Generation date: 2025-05-06T13:42:37
# Schema: dcat-4C-ap
#
...
class SoftwareAgentId(InstrumentId):
    pass
...
class Instrument(YAMLRoot):
    """
    An entity, such as a device that has an active role within a dataset generating activity, because it was used as
    an instrument.
    """
    _inherited_slots: ClassVar[List[str]] = []
...
class Device(Instrument):
    """
    A hardware device with a certain function that was used within an Activity.
    """
    _inherited_slots: ClassVar[List[str]] = []
...
@dataclass(repr=False)
class SoftwareAgent(Instrument):
    """
    ...
    class_model_uri: ClassVar[URIRef] = NFDI4C.SoftwareAgent

    id: Union[str, SoftwareAgentId] = None

    def __post_init__(self, *_: List[str], **kwargs: Dict[str, Any]):
        if self._is_empty(self.id):
            self.MissingRequiredField("id")
        if not isinstance(self.id, SoftwareAgentId):
            self.id = SoftwareAgentId(self.id)
